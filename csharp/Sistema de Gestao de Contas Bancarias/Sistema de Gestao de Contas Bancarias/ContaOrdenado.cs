namespace Sistema_de_Gestao_de_Contas_Bancarias
{
    ...
    // ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: //
    internal class ContaOrdenado(string numero, string titular, decimal saldo, decimal limite)
        : Conta(numero, titular, saldo)
    {
        public decimal LimiteDescoberto { get; set; } = limite;

        public override void Levantar(decimal valor)
        {
            if (valor <= 0) throw new ArgumentException("O valor deve ser positivo.");
            // Permite levantar até Saldo + Limite
            if (valor > (Saldo + LimiteDescoberto))
                throw new InvalidOperationException("Operação recusada: Excede o limite de descoberto.");

            // Lógica interna para subtrair do saldo (pode ficar negativo)
            typeof(Conta).GetProperty("Saldo").SetValue(this, Saldo - valor);
            Console.WriteLine($"✔ Levantamento de {valor:C2} efetuado (Uso de crédito/descoberto).");
        }

        public override void ExibirExtrato()
        {
            base.ExibirExtrato();
            Console.WriteLine($"Limite de Descoberto: {LimiteDescoberto:C2}");
        }
    }
}

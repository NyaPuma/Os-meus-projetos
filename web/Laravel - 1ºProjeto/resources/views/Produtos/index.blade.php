                ...
                <div class="row g-2">
                    <div class="col-sm-6 col-md-3">
                        ...
                        <select name="estado" id="estado" class="form-select form-select-sm"
                            ...
                            <option value="ativo" {{ request('estado') == 'ativo' ? 'selected' : '' }}>Disponível
                            </option>
                            <option value="inativo" {{ request('estado') == 'inativo' ? 'selected' : '' }}>Indisponível
                            </option>
                        </select>
                    </div>

                    <div class="col-sm-6 col-md-3">
                        <label for="stock_status" class="form-label small fw-bold text-muted mb-1">Alertas de
                            Stock</label>
                        <select name="stock_status" id="stock_status" class="form-select form-select-sm"
                            onchange="this.form.submit()">
                            <option value="">Todos os Níveis</option>
                            <option value="com_stock" {{ request('stock_status') == 'com_stock' ? 'selected' : '' }}>Com
                                Stock</option>
                            <option value="alerta" {{ request('stock_status') == 'alerta' ? 'selected' : '' }}>Stock
                                Crítico / Mínimo</option>
                        </select>
                    </div>
                    ...
                    <div class="col-sm-6 col-md-3">
                        ...
                        <select name="order_by" id="order_by" class="form-select form-select-sm"
                            ...
                            <option value="referencia" {{ request('order_by') == 'referencia' ? 'selected' : '' }}>
                                Referência</option>
                            <option value="preco_venda" {{ request('order_by') == 'preco_venda' ? 'selected' : '' }}>
                                Preço de Venda</option>
                            <option value="portes_envio" {{ request('order_by') == 'portes_envio' ? 'selected' : '' }}>
                                Portes de Envio</option>
                            <option value="stock_atual" {{ request('order_by') == 'stock_atual' ? 'selected' : '' }}>
                                Quantidade em Stock</option>
                ...
                @if ($produtos->hasPages())
                    <div class="row align-items-center mt-4">
                        <div class="col-12 col-md-4 text-center text-md-start text-muted small mb-2 mb-md-0">
                            A mostrar {{ $produtos->firstItem() }} a {{ $produtos->lastItem() }} de
                            {{ $produtos->total() }} entradas
                        </div>

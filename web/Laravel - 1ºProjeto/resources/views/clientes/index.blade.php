            ...
            <form action="{{ route('clientes.index') }}" method="GET">
                ...
                <div class="input-group mb-3">
                    <input type="text"
                           ...
                           id="search"
                           class="form-control"
                           placeholder="Procurar por Nome, Email, NIF..."
                ...
                <div class="row g-2">
                    <div class="col-sm-6">
                        ...
                        <select name="order_by" id="order_by" class="form-select form-select-sm" onchange="this.form.submit()">
                            ...
                            <option value="nif" {{ request('order_by') == 'nif' ? 'selected' : '' }}>NIF</option> <option value="email" {{ request('order_by') == 'email' ? 'selected' : '' }}>Email</option>
                            <option value="telefone" {{ request('order_by') == 'telefone' ? 'selected' : '' }}>Telefone</option>
                            <option value="estado" {{ request('order_by') == 'estado' ? 'selected' : '' }}>Estado</option> </select>
                    </div>
                    ...
                    <div class="col-sm-6">
                        ...
                        <select name="direction" id="direction" class="form-select form-select-sm" onchange="this.form.submit()">
                            <option value="asc" {{ request('direction', 'asc') == 'asc' ? 'selected' : '' }}>Crescente (A-Z / Menor ID)</option>
                            <option value="desc" {{ request('direction') == 'desc' ? 'selected' : '' }}>Decrescente (Z-A / Maior ID)</option>
                        </select>
                    </div>
                </div>
                ...
                <table class="table table-striped table-hover table-bordered align-middle mb-0">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th>Telefone</th>
                            <th>País</th> <th>Estado</th> <th width="240">Ações</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($clientes as $cliente)
                            <tr>
                                <td>{{ $cliente->id }}</td>
                                <td>{{ $cliente->nome }}</td>
                                <td>{{ $cliente->nif ?? '---' }}</td> <td>{{ $cliente->email }}</td>
                                <td>{{ $cliente->telefone }}</td>
                                <td>{{ $cliente->pais ?? 'PT' }}</td> <td> @if(($cliente->estado ?? 'ativo') == 'ativo')
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center"> Nenhum cliente encontrado com os critérios aplicados.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
